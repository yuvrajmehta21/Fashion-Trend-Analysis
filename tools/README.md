...
| `analyze_trends.py` | pandas: new-this-week, rising attributes, sell-through, search velocity, cross-source corroboration. | `data/catalog.json`, `.tmp/keywords_<date>.json` | `.tmp/trends_<date>.json` |
...
`--run-date` (date to stamp/analyze), `--input PATH` (override the auto-picked input).
